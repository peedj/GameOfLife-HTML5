function LifeAppController($scope) {
    $scope.initVariables = function() {
        ...
        $scope.games = {
            ...
            "Four gliders": {"44": {"43": 1, "44": 1, "49": 1, "50": 1, "51": 0}, "45": {"43": 1, "45": 1, "48": 1, "50": 1}, "46": {"43": 1, "48": 0, "50": 1}, "47": {"48": 0}, "48": {"48": 0}, "49": {"48": 0}, "52": {"43": 1, "44": 0, "45": 0, "50": 1}, "53": {"43": 1, "44": 0, "45": 1, "46": 0, "48": 1, "50": 1}, "54": {"43": 1, "44": 1, "45": 0, "49": 1, "50": 1}}
        };

        $scope.currentStates = angular.copy($scope.games["Four gliders"]); // Four gliders
    ...
    $scope.runNextGeneration = function(force) {
    ...
    $scope.game = {
        ...
        loadGame: function(data) {
            if (!confirm("All unsaved changes will be lost. Continue?"))
                return;
            $scope.currentStates = angular.copy(data);
            $scope.game.restartScene();
        },
        pause: function() {
            $scope.gameStatus = 0;
        ...
        run: function() {
            $scope.gameStatus = 1;
            $scope.interval && clearInterval($scope.interval);
            $scope.interval = setInterval($scope.runNextGeneration, Math.round($scope.gameInterval / $scope.s));
        },
        updateSpeed: function() {
            $scope.gameStatus && $scope.game.run();
        },
